...
**Date:** 2026-09-15
**Media type:** `application/vnd.tool-dictionary+json; v=0.1`

A portable file format for describing a large set of callable tools, and an HTTP
API for searching it, so that an agent can find the two or three tools it needs
among thousands without ever seeing the rest.
...
Error bodies: `{ "error": { "code": "…", "message": "…", "details": [] } }`.
...
them. They stay available to the integrator via the API.
...
The last sentence is present only when `list_tools` is exposed.
...
```json
{
  ...
  "tools": [ { "name": "search_tools", … }, { "name": "list_tools", … } ],
...
### 16.1 The service never executes

A Tool Dictionary server MUST NOT call any endpoint described by an entry. It
fetches only: its own configured `sources`, and branch URLs. Both are
configuration, not user input, and SHOULD be allow-listed.
...
7. Never calls an entry's endpoint.
